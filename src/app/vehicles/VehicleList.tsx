      ...
      >
        {filteredVehicles.map((vehicle, i) => (
          <Card key={i} className="rounded-xl overflow-hidden shadow-md">
            <div className="relative h-48 w-full"> {/* Aspect ratio for images */}
              <Image
            ...
            <div className="p-6">
              <h2 className="text-lg font-semibold mb-2">{vehicle.car_name}</h2>
              <p className="text-gray-600 mb-4">{vehicle.short_description}</p>
              <a 
                href={vehicle.turo_url}
                target="_blank"
                rel="noopener noreferrer"
                className="block w-full bg-sky-500 text-white text-center py-2 rounded-md hover:bg-sky-600"
              >
                Check Availability
              </a>
            </div>
